...
c-----------------------------------------------------------------------
      ...
      do i=1,npts
         ...
         myscn2(i,1) = ex_cn
         myscn2(i,2) = ex_cp
         myscn1(i) = sin(alpha*pi*xx) ! ex sol for phi  
      enddo
...
c-----------------------------------------------------------------------
      ...
      if (ncemface_pec(2).ge.1)  then ! second field  cn 
         ...
         do i=1,ncemface_pec(2)
            ...
            if (abs(xx+1).lt.eps) then
               cn(i0,1) = exp(1.5+sin(alpha*pi*xx))
            elseif (abs(xx-1).lt.eps) then
               cn(i0,1) = exp(1.5+sin(alpha*pi*xx))
            endif
         enddo
      endif
      ...
      if (ncemface_pec(3).ge.1)  then ! third field cp
         ...
         do i=1,ncemface_pec(3)
            ...
            if (abs(xx+1).lt.eps) then ! xx=-1
               cN(i0,2) = exp(2.0-sin(alpha*pi*xx))
            elseif (abs(xx-1).lt.eps) then
               cN(i0,2) = exp(2.0-sin(alpha*pi*xx))
            endif
         enddo
...
c... can be modified for inhomogeneous cases
      if (ncemface_nmn(1).gt.0) then   ! potent
        do i = 1,ncemface_nmn(1)
          ...
          xx = xm1(i0,1,1,1)
          if (abs(xx+1).lt.eps) then ! xx=-1
            bc_flux(j,1) = -1.0*alpha*pi*cos(alpha*pi*xx) ! note: normal vec "-1", Lan
          elseif (abs(xx-1).lt.eps) then
            bc_flux(j,1) =  1.0*alpha*pi*cos(alpha*pi*xx) ! flux bcs only physical value
          endif
        enddo
      endif
...
c---------------------------------------------------------------------
     ...
     $                , phip(lt),phix(lt),phiy(lt),phiz(lt)
      ...
      do i= 1,npts 
         ...
         charge(i)=  alpha*alpha*pi*pi*sin(alpha*pi*xx) - tmp2 - tmp3 ! ex sol for Q  
